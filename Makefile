...
acceptance_test = mvn test -Dtest="$(1).AcceptanceTests*" -pl \
ReferenceSocketClient
...
maven_release = mvn build-helper:parse-version -B release:prepare -DskipTests \
-Darguments=-DskipTests -DreleaseVersion=v$(1) -DdevelopmentVersion=$(2)
...
get_major_version = $(shell mvn build-helper:parse-version  \
help:evaluate -Dexpression=parsedVersion.majorVersion -q -DforceStdout)

get_minor_version = $(shell mvn build-helper:parse-version  \
help:evaluate -Dexpression=parsedVersion.minorVersion -q -DforceStdout)

get_patch_version = $(shell mvn build-helper:parse-version  \
help:evaluate -Dexpression=parsedVersion.incrementalVersion -q -DforceStdout)

get_project_version = \
v$(get_major_version).$(get_minor_version).$(get_patch_version)

get_running_docker_containers = docker container ls -q

docker_arguments = java -jar RobotWorldsServer.jar -p 5050 $(1) $(2)

run_docker_image = docker run -p 5000:5050 robot-worlds-socket-server:latest \
$(1) & echo "Running docker image..."
...
docker_build: maven_clean maven_package
	docker compose build

docker_release: docker_build test_docker
	docker login gitlab.wethinkco.de:5050
	docker tag robot-worlds-socket-server:latest \
gitlab.wethinkco.de:5050/mxomagub021/gerald_lawson/socket-server:\
$(get_project_version)
	docker tag robot-worlds-api-server:latest \
gitlab.wethinkco.de:5050/mxomagub021/gerald_lawson/api-server:\
$(get_project_version)
	docker push \
gitlab.wethinkco.de:5050/mxomagub021/gerald_lawson/socket-server:\
$(get_project_version)
